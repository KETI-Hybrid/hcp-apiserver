...
func (SetManagementResource) Uri() string {
	return "/gke/locations/cluster/nodePools/setManagement"
}
...
func (SetManagementResource) Post(rw http.ResponseWriter, r *http.Request, ps httprouter.Params) apis.Response {
	...
	resp, err := containerService.Projects.Locations.Clusters.NodePools.SetManagement(name, realRequest).Context(ctx).Do()
	if err != nil {
		klog.Errorln(err)
	}
