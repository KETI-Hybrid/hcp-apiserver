...
func (SetNetworkPolicyResource) Uri() string {
	return "/gke/locations/cluster/setNetworkPolicy"
}

func (SetNetworkPolicyResource) Post(rw http.ResponseWriter, r *http.Request, ps httprouter.Params) apis.Response {
	...
	resp, err := containerService.Projects.Locations.Clusters.SetNetworkPolicy(name, realRequest).Context(ctx).Do()
	if err != nil {
		klog.Errorln(err)
	}
