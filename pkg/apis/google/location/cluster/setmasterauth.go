...
func (SetMasterAuthResource) Uri() string {
	return "/gke/locations/cluster/setMasterAuth"
}

func (SetMasterAuthResource) Post(rw http.ResponseWriter, r *http.Request, ps httprouter.Params) apis.Response {
	...
	resp, err := containerService.Projects.Locations.Clusters.SetMasterAuth(name, realRequest).Context(ctx).Do()
	if err != nil {
		klog.Errorln(err)
	}
