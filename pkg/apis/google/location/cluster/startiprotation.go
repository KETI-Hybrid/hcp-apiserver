...
func (StartIpRotationResource) Uri() string {
	return "/gke/locations/cluster/startIpRotation"
}

func (StartIpRotationResource) Post(rw http.ResponseWriter, r *http.Request, ps httprouter.Params) apis.Response {
	...
	resp, err := containerService.Projects.Locations.Clusters.StartIpRotation(name, realRequest).Context(ctx).Do()
	if err != nil {
		klog.Errorln(err)
	}
